					...
					int height, int split)
{
	...
	for (int row = start_row; row < end_row; ++row) {
		...
		auto result = get_start_point(bin_img, w, h, &start_pt, 1,
					      w - 1, 1, h - 1, "horizontal");
...
// ==================== 准备入环 ====================
std::tuple<Point, Point> *prepare_enter_ring(
	const uint8_t *bin_img, int width, int height, StatusSwitcher *ss,
	const std::tuple<Point, Point> *start_point, int jump_threshold,
	int scan_start_height, int scan_row_step, int center_distance_threshold)
{
	int h = height;
	int w = width;
	...
	for (size_t i = 1; i < line_length_buffer.size(); ++i) {
		...
		if (curr_width - prev_width > jump_threshold) {
			// 计算前后两行的中心点
			Point prev_center((lp_prev.x + rp_prev.x) / 2,
					  (lp_prev.y + rp_prev.y) / 2);
			Point curr_center((lp_curr.x + rp_curr.x) / 2,
					  (lp_curr.y + rp_curr.y) / 2);

#ifdef SMTC2GO_DEBUG
			LOG_DEBUG("线长变化：%d", curr_width - prev_width);
			LOG_DEBUG("突变前中点: (%d, %d), 突变后中点: (%d, %d)",
				  prev_center.x, prev_center.y, curr_center.x,
				  curr_center.y);
#endif

			// 根据中心点偏移方向判断左右
			if (prev_center.x - curr_center.x >
			...
			} else if (prev_center.x - curr_center.x <
				   -center_distance_threshold) {
...
// ==================== 准备出环 ====================
std::tuple<Point, Point> *prepare_exit_ring(
	const uint8_t *bin_img, int width, int height, StatusSwitcher *ss,
	const std::tuple<Point, Point> *start_point, int jump_threshold,
	int scan_stop_height, int scan_row_step, int center_distance_threshold)
{
	int h = height;
	int w = width;
	...
	for (size_t i = 1; i < line_length_buffer.size(); ++i) {
		...
		if (curr_width - prev_width > jump_threshold) {
			...
			cv::Mat debug_img(height, width, CV_8UC1,
					  const_cast<uint8_t *>(bin_img));
			cv::line(debug_img,
				 cv::Point(prev_center.x, prev_center.y),
				 cv::Point(curr_center.x, curr_center.y),
				 cv::Scalar(0), 2);

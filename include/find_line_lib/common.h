			...
			const char *fmt, ...)
{
	...
	fprintf(stderr, "[%s] %s:%d: ", level_names[static_cast<int>(level)],
...
#define LOG_DEBUG(fmt, ...) \
	log_message(LogLevel::Debug, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...) \
	log_message(LogLevel::Info, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...) \
	log_message(LogLevel::Warn, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...) \
	log_message(LogLevel::Error, __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#else
#define LOG_DEBUG(fmt, ...)
#define LOG_INFO(fmt, ...)
#define LOG_WARN(fmt, ...)
...
struct PerfTraceAccumulator {
	...
	void report() const
	{
		...
		for (int i = 0; i < size; ++i) {
			fprintf(stderr,
				"[性能] %-20s 平均 %8.3f 毫秒 (共 %d 次)\n",
				entries[i].name,
				entries[i].total_ms / entries[i].count,
				entries[i].count);
		}
		fprintf(stderr, "==========================\n\n");
	}
